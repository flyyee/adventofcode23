...
use std::{collections::HashMap, fs, cmp};
...
fn main() {
    ...
    for (mut pos, mut direction) in (0..height)
        .map(|x| ((x, -1), Direction::Right))
        .chain((0..height).map(|x| ((x, width), Direction::Left)))
        .chain((0..width).map(|x| ((-1, x), Direction::Down)))
        .chain((0..width).map(|x| ((height, x), Direction::Up)))
    {
        ...
        best = cmp::max(best, energized_count);
    }
