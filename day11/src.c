...
int binary_search(void *value, void *container, unsigned long element_size, unsigned long length, int (*comp)(void *, void *))
{
    unsigned long low = 0, high = length - 1;
    while (low <= high)
    {
        unsigned long mid = low + (high - low) / 2;
        int comp_result = (*comp)(value, container + mid * element_size);
        if (comp_result > 0) // v > e
        {
            if (mid == length - 1)
            {
                return length;
        ...
        else if (comp_result < 0) // e < v
        {
            ...
            {
                if ((*comp)(value, container + (mid - 1) * element_size) > 0)
                {
                    return mid - 1;
                }
                high = mid - 1;
...
{
    return abs(g1->row - g2->row) + abs(g1->col - g2->col);
}

void test1()
{
    vector v;
    vector_new(&v, sizeof(int), 0);
    int q = 5;
    int r = 6;
    int s = 8;
    int t = 69;
...
int main()
{
    ...
    // // Debug
    // puts("Empty rows:");
    // for (unsigned long i = 0; i < empty_rows.length; i++)
    // {
    //     printf("%d\n", *(int *)vector_at(&empty_rows, i));
    // }
    // puts("Empty cols:");
    // for (unsigned long i = 0; i < empty_cols.length; i++)
    // {
    //     printf("%d\n", *(int *)vector_at(&empty_cols, i));
    // }
    ...
    // {
    //     Galaxy *g = (Galaxy *)vector_at(&galaxies, i);
    //     printf("%d %d\n", g->x, g->y);
    // }

    // 4. Modify galaxy positions based on expansion
    int galaxy_row_number = -1;
    ...
    for (unsigned long i = 0; i < galaxies.length; i++)
    {
        ...
        p_galaxy->col += empty_cols_prior;
        p_galaxy->row += empty_rows_prior;
    }
